...
That log summarizes the local 93-map top100 audit, summary CPU training,
parameter sweep, seed-stability runs, forest core/burst/weighting and
seed-stability runs, and current next training decision.
...
artifacts, plus the source git commit/branch/dirty state, so downloaded results
are easier to verify later. The notebook also displays the decision summary's
revision and calibration columns plus each run's
